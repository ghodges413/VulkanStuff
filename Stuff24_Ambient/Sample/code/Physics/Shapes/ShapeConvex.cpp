...
static void BuildTetrahedron( const Vec3 * verts, const int num, std::vector< Vec3 > & hullPts, std::vector< tri_t > & hullTris ) {
	...
	int idx = FindPointFurthestInDir( verts, num, Vec3( 1, 1, 1 ) );
	points[ 0 ] = verts[ idx ];
	//int idx2 = FindPointFurthestInDir( verts, num, points[ 0 ] * -1.0f );
	int idx2 = FindPointFurthestInDir( verts, num, Vec3( -1, -1, -1 ) );
	assert( idx != idx2 );
	points[ 1 ] = verts[ idx2 ];
...
void ShapeConvex::Build( const Vec3 * pts, const int num ) {
	...
	assert( m_centerOfMass.IsValid() );
	if ( !m_centerOfMass.IsValid() ) {
		printf( "ShapeConvex: Center of Mass is invalid\n" );
	}
