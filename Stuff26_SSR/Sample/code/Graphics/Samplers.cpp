...
VkSampler Samplers::m_samplerRepeat;
VkSampler Samplers::m_samplerDepth;
VkSampler Samplers::m_samplerNearest;
VkSampler Samplers::m_samplerLightProbe;
...
bool Samplers::InitializeSamplers( DeviceContext * device ) {
	...
	result = vkCreateSampler( device->m_vkDevice, &samplerInfo, nullptr, &m_samplerRepeat );
	if ( VK_SUCCESS != result ) {
		printf( "failed to create m_samplerStandard!\n" );
		assert( 0 );
		return false;
	...
	result = vkCreateSampler( device->m_vkDevice, &samplerInfo, nullptr, &m_samplerNearest );
	if ( VK_SUCCESS != result ) {
		printf( "failed to create m_samplerStandard!\n" );
		assert( 0 );
		return false;
	...
	result = vkCreateSampler( device->m_vkDevice, &samplerInfo, nullptr, &m_samplerLightProbe );
	if ( VK_SUCCESS != result ) {
		printf( "failed to create m_samplerStandard!\n" );
		assert( 0 );
		return false;
...
void Samplers::Cleanup( DeviceContext * device ) {
	...
	vkDestroySampler( device->m_vkDevice, m_samplerRepeat, nullptr );
	vkDestroySampler( device->m_vkDevice, m_samplerDepth, nullptr );
	vkDestroySampler( device->m_vkDevice, m_samplerNearest, nullptr );
	vkDestroySampler( device->m_vkDevice, m_samplerLightProbe, nullptr );
}