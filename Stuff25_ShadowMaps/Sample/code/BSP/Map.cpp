...
bool LoadMap( DeviceContext * device ) {
	...
	//const char * fileName = "../../common/data/maps/testmap.map";
	const char * fileName = "../../common/data/maps/arena_test.map";
	//const char * fileName = "../../common/data/maps/arena_test_bad.map";
	...
	g_quakeModel.m_vertices = g_quakeVerts;
	std::vector< unsigned int > indices;
	for ( int i = 0; i < g_quakeIndices.size(); i++ ) {
	...
	for ( int i = 0; i < g_quakeModel.m_vertices.size(); i++ ) {
		bounds.Expand( g_quakeModel.m_vertices[ i ].pos );
	}
	printf( "Map Bounds: %f %f %f    %f %f %f\n",
...
void DrawMap( VkCommandBuffer cmdBuffer ) {
	g_quakeModel.DrawIndexed( cmdBuffer );
}
