...
void main() {
    ...
    // Map the floating point HDR color to the limited 256 levels
	//outColor.rgb    = vec3( 1.0 ) - exp( -exposure * color.rgb );
	outColor.rgb	= normal.rgb;
	outColor.rgb	= fragment.camPos.rgb;
	outColor.rgb	= fragment.dirToSun.rgb;
	//outColor.rgb	= reflection.rgb;
	outColor.rgb	= finalColor.rgb;
	float nearDist = 500.0f;
	float farDist = 2000.0f;
	t = ( farDist - dist ) / ( farDist - nearDist );
    outColor.a		= max( 0.0f, min( 1.0f, t ) );
	outColor.a = 1.0;
	//outColor.rgb	= normals[ 0 ].rgb;
/*
	outColor.r = 0.0f;
	outColor.g = 0.0f;
	outColor.b = -dirToSun.b;
	outColor.rgb	= fragment.camPos.rgb;
/*
	outColor.rgb	= v_worldSpacPos.rgb;
	outColor.r = 0.0f;
	outColor.g = 0.0f;
	outColor.b += 10.0f;
	outColor.b /= 10.0f;
