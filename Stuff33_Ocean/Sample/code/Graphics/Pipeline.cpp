...
bool Pipeline::Create( DeviceContext * device, const CreateParms_t & parms ) {
	...
	switch ( parms.depthCompare ) {
		default:
		case DEPTH_COMPARE_LEQUAL: depthStencil.depthCompareOp = VK_COMPARE_OP_LESS_OR_EQUAL; break;
		case DEPTH_COMPARE_LESS: depthStencil.depthCompareOp = VK_COMPARE_OP_LESS; break;
		case DEPTH_COMPARE_EQUAL: depthStencil.depthCompareOp = VK_COMPARE_OP_EQUAL; break;
		case DEPTH_COMPARE_GREATER: depthStencil.depthCompareOp = VK_COMPARE_OP_GREATER_OR_EQUAL; break;
		case DEPTH_COMPARE_GEQUAL: depthStencil.depthCompareOp = VK_COMPARE_OP_GREATER; break;
		case DEPTH_COMPARE_NEVER: depthStencil.depthCompareOp = VK_COMPARE_OP_NEVER; break;
		case DEPTH_COMPARE_ALWAYS: depthStencil.depthCompareOp = VK_COMPARE_OP_ALWAYS; break;
		case DEPTH_COMPARE_NEQUAL: depthStencil.depthCompareOp = VK_COMPARE_OP_NOT_EQUAL; break;
	};
	...
	if ( parms.blendMode == BLEND_MODE_ALPHA ) {
		...
		colorBlendAttachment.alphaBlendOp = VK_BLEND_OP_ADD;
	} else if ( parms.blendMode == BLEND_MODE_MULTIPLY ) {
		colorBlendAttachment.blendEnable = VK_TRUE;

		colorBlendAttachment.srcColorBlendFactor = VK_BLEND_FACTOR_ONE;
		colorBlendAttachment.dstColorBlendFactor = VK_BLEND_FACTOR_ONE;
		colorBlendAttachment.colorBlendOp = VK_BLEND_OP_MULTIPLY_EXT;
	...
	// This is for MRT.  If a framebuffer has multiple render targets but doesn't setup an equal number of
	...
	if ( m_parms.numColorAttachments > 1 ) {
