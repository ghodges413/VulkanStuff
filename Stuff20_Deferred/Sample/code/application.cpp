...
void Application::DrawFrame() {
	...
	for ( int i = 0; i < GFrameBuffer::s_numColorImages; i++ ) {
		g_gbuffer.m_imageColor[ i ].TransitionLayout( cmdBuffer, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL );
	}
	g_gbuffer.m_imageDepth.TransitionLayout( cmdBuffer, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL );
	g_gbuffer.BeginRenderPass( &m_device, imageIndex );

	DrawGBuffer( parms );

	g_gbuffer.EndRenderPass( &m_device, imageIndex );
	for ( int i = 0; i < GFrameBuffer::s_numColorImages; i++ ) {
		g_gbuffer.m_imageColor[ i ].TransitionLayout( cmdBuffer, VK_IMAGE_LAYOUT_GENERAL );
	}
	g_gbuffer.m_imageDepth.TransitionLayout( cmdBuffer, VK_IMAGE_LAYOUT_DEPTH_READ_ONLY_STENCIL_ATTACHMENT_OPTIMAL );
...
// 	g_offscreenFrameBuffer.BeginRenderPass( &m_device, imageIndex );
// 
