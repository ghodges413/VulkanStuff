...
bool Application::InitializeVulkan() {
	...
	{
		Model model;
		const bool didLoad = model.LoadOBJ( "../common/data/meshes/static/companionCube.obj" );
		if ( !didLoad ) {
	...
	//
	{
		// Create Descriptor Pool
		{
			...
			poolSizes[ 1 ].descriptorCount = 1 * m_numDescriptorSets;
		...
		// Create Descriptor Set Layout
		{
			...
			std::array< VkDescriptorSetLayoutBinding, 3 > bindings = { uboCameraLayoutBinding, uboModelLayoutBinding, samplerLayoutBinding };
	...
	//
	InitializeOceanSimulation( m_vkDevice );
	CreateTextureOceanSimulation( m_vkPhysicalDevice, m_vkDevice, m_vkGraphicsQueue, m_vkCommandBuffers[ 0 ] );
...
void Application::Cleanup() {
	// Destroy compute shader related
	CleanupOceanSimulation( m_vkDevice );
...
void Application::DrawFrame() {
	...
	{
		Vec3d camPos = Vec3d( 20, 0, 0 );
		Vec3d camLookAt = Vec3d( 0, 0, 0 );
	...
	// Record Draw Commands
	{
		...
		// Run our compute shader here
		UpdateOceanSimulation( m_vkDevice, m_vkCommandBuffers[ imageIndex ] );
		...
		for ( int i = 0; i < m_entities.size(); i++ ) {
			...
			// Issue Draw Commands
			{
				...
				const int descriptorIdx = numDescriptorSetsUsed;
				{
					...
					extern VkImageView g_vkTextureImageViewCompute;
					extern VkSampler g_vkTextureSamplerCompute;
					...
					descriptorWrites[ 2 ].dstSet = m_vkDescriptorSets[ numDescriptorSetsUsed ];
